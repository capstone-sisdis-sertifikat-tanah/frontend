...
export function AktaTanahPDF({ identifier }: { identifier: string }) {
  return (
...
export function AktaTanahPreview({ identifier }: { identifier: string }) {
  return (
    <PDFViewer className="w-full h-[calc(100dvh-140px)]">
      <AktaTanahPDF identifier={identifier} />
    </PDFViewer>
  );
}

export function AktaTanah({ id }: { id: string }) {
  const { trigger, data, isMutating } = useMutation(`/akta/identifier/${id}`);
    ...
    <>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} static={true} className="z-[100]">
        <DialogPanel className="sm:max-w-2xl grid gap-4">
          <AktaTanahPreview identifier={identifier} />
          <div className="flex justify-end">
            <PDFDownloadLink document={<AktaTanahPDF identifier={identifier} />} fileName="aktaTanah-tanah.pdf">
