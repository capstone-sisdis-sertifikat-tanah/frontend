...
type CreateAktaTanahApiPayload = {
  idSupplyChain: string;
  idDivisiPenerima: string;
  idTransportasi: string;
  beratMuatan: string;
...
export function CreateAktaTanah() {
  ...
  const methods = useForm<CreateAktaTanahPayload>({
    shouldUnregister: true,
  });

  const { register, handleSubmit, reset, resetField, control, watch } = methods;

  const { trigger, isMutating } = useMutation<CreateAktaTanahApiPayload>("/akta");

  const { mutate } = useOptimisticList(`/akta/${id}`);

  const onSubmit = async (payload: CreateAktaTanahPayload) => {
    try {
      await trigger({
        ...payload,
