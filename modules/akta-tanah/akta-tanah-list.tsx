...
const placeholderProps = {
  ...
  description: "Setiap akta tanah tanah akan tercatat disini.",
};
...
export function AktaTanahList() {
  ...
  const { data, isLoading } = useSWR<{ data: Array<AktaTanah> }>(`/akta-tanah/pemilik/${id}`);
  ...
  return (
    <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {data?.data.map((aktaTanah) => {
        const isWaitingBerangkat =
          new Date(aktaTanah.waktuBerangkat) > new Date() && aktaTanah.status === "Need Approval";

        return (
          <Link href={`/aktaTanah/${aktaTanah.id}`} key={aktaTanah.id}>
            <Card className="group px-4 pt-5 pb-1">
              ...
              <div className="mt-2 grid place-items-end divide-x divide-tremor-border border-t border-tremor-border dark:divide-dark-tremor-border dark:border-dark-tremor-border">
                <div className="py-2">
                  ...
                  >
                    {isWaitingBerangkat ? "Menunggu Waktu Berangkat" : statusText[aktaTanah.status]}
                  </p>
                </div>
              </div>
              <span
